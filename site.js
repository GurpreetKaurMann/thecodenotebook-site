...
api.onUser(async (user) => {
  ...
  if (page === "admin" && user) renderAdmin(user);
...
async function renderDashboard(user) {
  ...
  // the owner gets a link to the admin page; everyone else never sees it
  api.isAdmin?.(user.uid).then((ok) => { if (ok) $("#dash-admin")?.removeAttribute("hidden"); }).catch(() => {});
...
async function renderAdmin(user) {
  const sub = $("#admin-sub"), msg = $("#admin-msg"), body = $("#admin-body");
  const ok = await api.isAdmin(user.uid).catch(() => false);
  if (!ok) {
    sub.textContent = "";
    show(msg, "This page is for the site owner. If that's you, add your user ID to the \"admins\" collection in Firebase — the steps are in SETUP-GUIDE.md.");
    return;
  }
  sub.textContent = "Signed in as " + (user.email || user.name);

  let members = [];
  const fmt = (d) => d ? d.toLocaleDateString("en-IN", { day: "numeric", month: "short", year: "numeric" }) : "—";
  const done = (m) => Object.values(m.progress).filter(Boolean).length;
  ...
  function paint() {
    ...
    $("#admin-rows").innerHTML = rows.length
      ? rows.map((m) => `<tr><td>${esc(m.name) || "<span class='muted'>—</span>"}</td><td>${esc(m.email)}</td>` +
          `<td>${esc(fmt(m.createdAt))}</td><td>${done(m)} / ${LESSONS.length}</td>` +
          `<td>${m.waitlist.length ? esc(m.waitlist.join(", ")) : "<span class='muted'>—</span>"}</td></tr>`).join("")
      : `<tr><td colspan="5" class="muted">No members match "${esc(term)}".</td></tr>`;
    $("#admin-count").textContent = `Showing ${rows.length} of ${members.length} members.`;
  }

  function stats() {
    const now = Date.now(), DAY = 86400000;
    const since = (days) => members.filter((m) => m.createdAt && now - m.createdAt.getTime() <= days * DAY).length;
    $("#st-total").textContent = members.length;
    $("#st-week").textContent = since(7);
    $("#st-month").textContent = since(30);
    $("#st-active").textContent = members.filter((m) => done(m) > 0).length;
    const tally = {};
    for (const m of members) for (const [id, v] of Object.entries(m.progress)) if (v) tally[id] = (tally[id] || 0) + 1;
    const top = Object.entries(tally).sort((a, b) => b[1] - a[1]).slice(0, 5);
    $("#st-lessons").innerHTML = top.length
      ? top.map(([id, n]) => `${esc((LESSONS.find((l) => l.id === id) || {}).title || id)} — <b>${n}</b>`).join(" &nbsp;·&nbsp; ")
      : "Nobody has ticked a lesson yet.";
  }

  async function load() {
    try {
      members = await api.listMembers();
      body.hidden = false; msg.hidden = true;
      stats(); paint();
    } catch (e) {
      show(msg, "Could not read the member list. Check that the Firestore rules from SETUP-GUIDE.md are published, and that your user ID is in the \"admins\" collection.");
    }
  }

  $("#admin-search").addEventListener("input", paint);
  $("#admin-refresh").addEventListener("click", load);
  $("#admin-csv").addEventListener("click", () => {
    const q = (v) => `"${String(v ?? "").replace(/"/g, '""')}"`;
    const csv = ["Name,Email,Joined,Lessons done,Course interest"]
      .concat(members.map((m) => [m.name, m.email, fmt(m.createdAt), done(m), m.waitlist.join(" ")].map(q).join(",")))
      .join("\n");
